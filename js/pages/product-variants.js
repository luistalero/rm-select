...
async function loadVariants() {
  ...
  const { data, error } = await supabase.from('product_variants').select('id,product_id,name,sku,price,attributes,is_active').eq('product_id', productId).order('created_at');
...
function showError(message) { const box = $('variant-form-error'); if (box) { box.textContent = message; box.hidden = false; } }
function clearForm() { $('variant-id').value = ''; $('variant-name').value = ''; $('variant-sku').value = ''; $('variant-price').value = ''; $('variant-attributes').value = ''; $('variant-active').checked = true; $('variant-form-error').hidden = true; $('variant-form').hidden = true; }
function openForm(variant = null) { $('variant-id').value = variant?.id || ''; $('variant-name').value = variant?.name || ''; $('variant-sku').value = variant?.sku || ''; $('variant-price').value = variant?.price ?? ''; $('variant-attributes').value = variant?.attributes && Object.keys(variant.attributes).length ? JSON.stringify(variant.attributes) : ''; $('variant-active').checked = variant ? Boolean(variant.is_active) : true; $('variant-form-error').hidden = true; $('variant-form').hidden = false; $('variant-name').focus(); }
...
async function saveVariant() {
  ...
  const button = $('save-variant'); button.disabled = true; button.textContent = 'Guardando…'; $('variant-form-error').hidden = true;
  try {
    ...
    let attributes = {};
    if (raw) { try { attributes = JSON.parse(raw); } catch { throw new Error('Los atributos deben ser JSON válido.'); } if (!attributes || Array.isArray(attributes) || typeof attributes !== 'object') throw new Error('Los atributos deben ser un objeto JSON.'); }
    ...
    clearForm(); await loadVariants();
    const status = $('product-status'); if (status) { status.hidden = false; status.textContent = id ? 'Variante actualizada correctamente.' : 'Variante creada y preparada para inventario.'; status.className = 'admin-status is-success'; }
  } catch (error) { console.error('[RM SELECT] variant save error:', error); showError(error?.code === '23505' ? 'Ya existe una variante con ese SKU o identificador.' : error?.message || 'No fue posible guardar la variante.'); }
  finally { button.disabled = false; button.textContent = 'Guardar variante'; }
}

async function toggleVariant(id) {
  const variant = variants.find((item) => item.id === id); if (!variant) return;
...
$('variants-list')?.addEventListener('click', (event) => {
  ...
  if (edit) { const variant = variants.find((item) => item.id === edit.dataset.editVariant); if (variant) openForm(variant); }
...
if (modal) {
  const observer = new MutationObserver(() => {
    if (!modal.hidden && $('product-id')?.value) {
      $('new-variant-button').disabled = false;
      loadVariants();
